...
function updateGame() {
    ...
    function getAdminSetting(key) {
        return JSON.parse(localStorage.getItem('admin_' + key) || 'false');
    }
    function checkAdminSkipWave() {
        if (localStorage.getItem('admin_skipWave') === 'true') {
            localStorage.setItem('admin_skipWave', 'false');
            if (typeof redWave !== 'undefined') {
                redWave++;
                spawnRedSpaceships();
            }
        }
    ...
    // Admin: transparent mode disables hitbox
    let spaceshipRect;
    if (getAdminSetting('transparent')) {
        spaceshipRect = { top: -1000, bottom: -900, left: -1000, right: -900 };
    } else {
        spaceshipRect = {
            top: spaceship.y + hitboxOffsetY,
            bottom: spaceship.y + hitboxOffsetY + hitboxHeight,
            left: spaceship.x + hitboxOffsetX,
            right: spaceship.x + hitboxOffsetX + hitboxWidth
        };
    }
    ...
    for (let i = 0; i < lasers.length; i++) {
        ...
        for (let j = 0; j < asteroids.length; j++) {
            ...
            if (isColliding(laserRect, asteroidRect)) {
                    let damage = getAdminSetting('oneshot') ? 1000 : 1;
                    if (asteroid.hitCount > damage) {
                        asteroid.hitCount -= damage;
    ...
    for (let i = 0; i < asteroids.length; i++) {
        ...
        if (isColliding(spaceshipRect, asteroidRect)) {
                if (!spaceship.invincible && !getAdminSetting('invincible')) {
                    let damage = 10;
                    if (asteroid.width >= 50) {
                        damage = asteroid.crackImg ? 15 : 20;
                    }
    ...
    for (let i = 0; i < redLasers.length; i++) {
        ...
        if (isColliding(spaceshipRect, redLaserHitbox)) {
                if (!spaceship.invincible && !getAdminSetting('invincible')) {
                    spaceship.health -= adjustDamage(damage);
                    triggerSpaceshipInvincibility();
                }
